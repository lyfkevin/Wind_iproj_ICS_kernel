...
clkwait:
	...
	{
        msleep(50); // driver patch about reset for when hostpot is being off
		resched = TRUE;
	}
...
extern char fw_path[MOD_PARAM_PATHLEN];	/* LGE_patch : firmware path */
extern char nv_path[MOD_PARAM_PATHLEN];	/* LGE_patch : nvram path */
	...
	if (flag == TRUE) {
...
#if 0	/* useless code : insmod lockup */
...
#endif
#if !defined(OOB_INTR_ONLY)	/* LGE_patch */
		...
		if (bus->dhd->dongle_reset) {
			...
			if (dhdsdio_probe_attach(bus, bus->dhd->osh, bus->sdh,
				...
				if(!bus->fw_path || !bus->nv_path){	/* LGE_patch */
					bus->fw_path = fw_path;
					bus->nv_path = nv_path;
				}
